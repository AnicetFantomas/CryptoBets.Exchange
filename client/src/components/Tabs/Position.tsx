...
const Orders = (props : any) => {
  ...
  return (
    <>
      <AppModal show={show} close={() => setShow(false)} title='Do you want to close this position ?'><ConfirmCancel close={() => setShow(false)} /></AppModal>
      <div className='flex justify-center w-full'>
        <div className='flex flex-col w-full'>

          <div className='flex flex-col w-full text-gray-200 '>
               
          {myPositions.length === 0 ? <div className='text-xl font-bold text-red-600'>No open positions yet !</div> :
                  <table  className='text-gray-200 '>
                    ...
                    <tbody>
                     ...
                     { myPositions.map((position:any) => (
                      <tr key={position._id} className='text-gray-500 my-0.5 text-center hover:text-gray-200'>
                        <td className='whitespace-nowrap '>{position._amountIn}</td>
                        <td className='pl-8 whitespace-nowrap'>{position._sizeDelta}</td>
                        <td className='pl-8 whitespace-nowrap'>{position._acceptablePrice * Math.pow(10, -40)}</td>
